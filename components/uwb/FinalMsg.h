...
public:
    ...
    static const std::size_t FRAME_SIZE = MHR_SIZE + COMMON_PAYLOAD_START_SIZE + 1 + FINAL_DATA_SIZE + 3*sizeof(uint64_t) + MFR_SIZE;
    typedef struct {
...
public:
    ...
    void getTimestamps(uint64_t* initial, uint64_t* response, uint64_t* final) const;
    void setTimestamps(uint64_t initial, uint64_t response, uint64_t final);
